...
trait JMocker extends JMockerExampleLifeCycle with HamcrestMatchers with JMockActions with AssertionListener { outer =>
  ...
  class JMockAction[T](v: T) {
    ...
    def willReturnEach(values: T*) = {
      will(new ActionSequence((values map { x: T => returnValue(x) }) : _*))
    }
...
trait JMockActions {
  ...
  def doAll(actions: Action*) = new DoAllAction(actions : _*)
  ...
  def onConsecutiveCalls(actions: Action*) = new ActionSequence(actions : _*)
}
